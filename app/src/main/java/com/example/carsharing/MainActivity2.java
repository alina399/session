...
public class MainActivity2 extends AppCompatActivity implements OnMapReadyCallback {
    ...
    private ArrayList<Trip_a> trips = new ArrayList();
    private ListView tripsList;
    private AdapterTrip tripAdapter;
    ...
    public void updateCars(GoogleMap googleMap) {
        for(int i = 0; i < jsonArray.length(); i++) {
            try {
                ...
                mMap.addMarker(new MarkerOptions().position(new LatLng(lat, lon)).title(model).snippet(String.valueOf(id)).icon(BitmapDescriptorFactory.fromResource(R.drawable.taxi)));
            } catch (JSONException e) {
                e.printStackTrace();
            }
        }
